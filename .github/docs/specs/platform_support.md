...
| macOS (CI) | Burst parallel `PtyCapture`/`CompleteAsync` under full-suite load can finish fast children before a transport pump blocks on the PTY master. | Transport pumps use `Task.Run`; post-exit quiet drain may close the master after a quiet period (same bounded grace as Windows). Do not add a caller-side pump wait loop (thread-pool starvation / cancel-before-kill). |
