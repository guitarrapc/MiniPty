...
| Linux / FreeBSD (`forkpty`) | Managed `file`/`argv`/`envp`/`cwd` UTF-8 blocks must stay allocated until the child has exec'd or failed; immediate free after `forkpty` races the child under parallel CI. | Native `forkpty` path keeps the exec payload alive on the backend until session disposal; the child calls `chdir(cwd)` on its fork copy-on-write mapping (no child-side `strdup`). |
| macOS (CI) | Burst parallel `PtyCapture`/`CompleteAsync` under full-suite load can finish fast children before a transport pump blocks on the PTY master. | macOS one-shot transport pumps queue via `UnsafeQueueUserWorkItem(preferLocal: true)`; post-exit quiet drain may close the master after a quiet period. Do not add a caller-side pump wait loop (thread-pool starvation / cancel-before-kill). |
