...
| Linux / FreeBSD (`forkpty`) | Managed `file`/`argv`/`envp`/`cwd` UTF-8 blocks must stay allocated until the child has exec'd or failed; immediate free after `forkpty` races the child under parallel CI. | Native `forkpty` path keeps the exec payload alive on the backend until session disposal; child copies `cwd` with `strdup` before `chdir`. |
