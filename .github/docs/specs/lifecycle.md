...
- **Do not block one-shot completion waiting for a transport pump on the caller thread.** A caller-side wait loop (busy-spin or `Thread.Sleep(0)`) can starve the thread pool under parallel CI load and can throw cancellation before `KillOnCancellation` runs. Transport pumps stay on `Task.Run`; post-exit quiet drain on Unix PTY masters bounds output loss when the slave closes early.
