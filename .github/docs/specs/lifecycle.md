...
- **Windows one-shot drain uses post-exit quiet heuristics, not completion truth.** After child exit, `AwaitPumpAsync` may close the ConPTY transport when transport reads have been quiet long enough, bounded by `OutputDrainGrace`. This unblocks EOF for blocked pumps; it does not detect shell command completion.
- **Do not block one-shot completion waiting for a transport pump on the caller thread.** A caller-side wait loop (busy-spin or `Thread.Sleep(0)`) can starve the thread pool under parallel CI load and can throw cancellation before `KillOnCancellation` runs. On macOS, one-shot transport pumps use `ThreadPool.UnsafeQueueUserWorkItem` with `preferLocal: true` so the blocking read starts promptly (node-pty attaches its reader synchronously at spawn); other platforms keep `Task.Run`. Post-exit quiet drain on Unix PTY masters bounds output loss when the slave closes early.
- **Linux `forkpty` exec payloads must outlive the pre-exec child.** The parent must keep `file`, `argv`, `envp`, and `cwd` pointers valid until the child has `chdir`/`execve` or exited. Freeing managed UTF-8 allocations immediately after `forkpty` returns races the child and flakes as exit **127** or wrong working-directory lookup under parallel CI load. Hold the payload until session/backend disposal; the fork child calls `chdir(cwd)` on its copy-on-write mapping (no child-side `strdup`—parent hold is the correctness fix). `posix_spawn` paths copy env/cwd in the kernel before returning.
