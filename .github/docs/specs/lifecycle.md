...
- **Unix one-shot transport pumps must block on the master before waiting for exit.** Fast children (for example `printf`) can finish and close the PTY slave while a thread-pool transport pump has not entered its read loop yet; macOS may then return exit code 0 with truncated output. One-shot completion must not call `WaitForExit` until the transport pump has entered its blocking read loop. The waiter must cooperatively yield (`Thread.Sleep(0)` or equivalent); busy-waiting is forbidden because it burns CPU and can starve the pump work item under parallel CI load.
- **Linux `forkpty` exec payloads must outlive the pre-exec child.** The parent must keep `file`, `argv`, `envp`, and `cwd` pointers valid until the child has `chdir`/`execve` or exited. Freeing managed UTF-8 allocations immediately after `forkpty` returns races the child and flakes as exit **127** or wrong working-directory lookup under parallel CI load. Hold the payload until session/backend disposal (after `execve`, the child no longer reads parent memory). `posix_spawn` paths copy env/cwd in the kernel before returning.
