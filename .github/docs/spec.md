...
| **macOS (ARM)** | A fixed `sleep` before `stty size` races with parent `Resize` on fast runners; the child may query size before `TIOCSWINSZ` / `SIGWINCH` (garbage like `SIZE:27280:28823`) | Child resize checks block on `read` until after `Resize`, then stdin unblocks via `CompleteAsync` input |
...
- **Child-visible resize tests must synchronize with the parent.** `PtySession.Resize` runs in the parent after `Pty.Start` returns; a child that probes size after a short `sleep` can win the race on fast hosts (notably macOS ARM CI). `stty size` then reports stale or nonsensical dimensions. Block the child on stdin (or another explicit handshake) until after `Resize`.
